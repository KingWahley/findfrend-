...
const INPUT_BASE = 'w-full rounded-2xl border border-slate-200 bg-white/80 px-4 py-3 text-sm shadow-sm outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200'
        ...
        <section className="w-full rounded-[32px] border border-white/60 bg-white/70 p-10 shadow-xl backdrop-blur">
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div className="max-w-xl space-y-4">
              <p className="text-xs uppercase tracking-[0.3em] text-slate-500">Random Friend Pro v2</p>
              <h1 className="text-4xl font-semibold text-slate-900 md:text-5xl">Spin the wheel. Meet a new friend.</h1>
              <p className="text-base text-slate-600">
                Drop in anonymously, set your vibe filters, and we will pull a friendly surprise from the community.
              </p>
            </div>
            <div className="flex w-full flex-col gap-3 md:w-auto">
              ...
              <p className="text-xs text-slate-500">No email needed. You can stay anonymous.</p>
            </div>
          </div>
        </section>
        {error ? (
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-8 px-6 py-12">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-slate-500">Random Friend Pro v2</p>
          <h1 className="text-4xl font-semibold text-slate-900">Find a surprise friend</h1>
        </div>
        <div className="rounded-3xl border border-white/60 bg-white/70 px-5 py-4 text-sm text-slate-600 shadow-sm">
          You are signed in as <span className="font-semibold text-slate-900">{profile.name}</span>
        </div>
      </header>

      <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-[32px] border border-white/60 bg-white/80 p-8 shadow-xl backdrop-blur">
          <h2 className="text-2xl font-semibold text-slate-900">Tune your spin</h2>
          <p className="mt-2 text-sm text-slate-600">Filters help match you with someone close to your vibe.</p>
          ...
          <div className="mt-6 grid gap-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <label className="text-xs uppercase tracking-[0.2em] text-slate-500">Minimum age</label>
                <input
                  type="number"
                  min="18"
                  max="99"
                  ...
                  onChange={(event) => setFilters({ ...filters, minAge: event.target.value })}
                />
              </div>
              <div className="space-y-2">
                <label className="text-xs uppercase tracking-[0.2em] text-slate-500">Maximum age</label>
                <input
                  type="number"
                  min="18"
                  max="99"
                  ...
                  onChange={(event) => setFilters({ ...filters, maxAge: event.target.value })}
                />
              </div>
            </div>
        ...
        <div className="rounded-[32px] border border-white/60 bg-white/80 p-8 shadow-xl backdrop-blur">
          <h2 className="text-2xl font-semibold text-slate-900">Your surprise match</h2>
          <p className="mt-2 text-sm text-slate-600">Spin to reveal a new face.</p>
          ...
          <div className="mt-6">
            {!match ? (
              ...
              <div className="flex flex-col items-center gap-4 rounded-3xl border border-emerald-100 bg-emerald-50 px-6 py-8 text-center shadow-sm">
                ...
                />
                <div>
                  <p className="text-lg font-semibold text-slate-900">{match.name}, {match.age}</p>
...
function ProfileSetup({ user, onDone }) {
  const [form, setForm] = useState({ name: '', city: '', age: '', phone: '', file: null })
  ...
  const uploadAvatar = async () => {
    ...
    const { error } = await supabase.storage.from('avatars').upload(path, form.file)
  ...
  return (
    <section className="w-full max-w-2xl rounded-[32px] border border-white/60 bg-white/80 p-8 shadow-xl backdrop-blur">
      ...
      <p className="mt-2 text-sm text-slate-600">Tell the community a bit about you before spinning.</p>
